...
layout = dbc.Container([
        ...
        dbc.Col(dbc.Card([
            dbc.CardBody([
                html.Div(id="buoy-controls-timeseries", children=[
                    ...
                    dcc.Dropdown(id="buoy-date-range",
                ...
                html.Div(id="buoy-controls-profile", style={"display":"none"}, children=[
                    html.Label("Display Period", style={"font-weight":"bold"}),
                    dcc.Dropdown(id="buoy-profile-date-range",
                                 options=DATE_RANGE_OPTIONS,
                                 value="1D"),
                    html.Hr(style={"border-top":"2px solid purple"}),
                    html.Label("Select Parameters", style={"font-weight":"bold"}),
    ...
    dbc.Modal([
        ...
        dbc.ModalBody([
            ...
            dcc.Dropdown(id="buoy-download-range",
                         options=DATE_RANGE_OPTIONS,
                         value="1D"),
            html.Br(),
            html.Label("Select Parameters:", style={"font-weight":"bold"}),
...
def _render_tab(tab, dr_ts, params_ts, dr_pf, params_pf):
    if tab == "tab-timeseries":
        ...
        return html.Div([
            dcc.Graph(figure=fig, style={"border":"2px solid lightgray","padding":"5px","height":"40vh"})
            for fig in figs
        ], style={"display":"flex","flexDirection":"column","gap":"10px"})

    # Vertical Profiles
    prof = buoy.fetch_profiles(dr_pf)
    if not prof:
    ...
    for p in params_pf:
        ...
        graphs.append(dcc.Graph(figure=fig, style={"border":"2px solid lightgray","padding":"5px","height":"40vh"}))
